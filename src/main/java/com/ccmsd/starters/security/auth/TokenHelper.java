...
public class TokenHelper
{
	...
	{
		long expiresIn = getExpiredIn(device);
		return new Date(timeProvider.now().getTime() + expiresIn * 1000);
	}

	public int getExpiredIn(Device device)
	{
		return (device != null && device.isMobile()) || (device != null && device.isTablet()) ? MOBILE_EXPIRES_IN
				: EXPIRES_IN;
	}
	...
	public Boolean validateToken(String token, UserDetails userDetails)
	{
		...
		return (username != null && username.equals(userDetails.getUsername())
				&& !isCreatedBeforeLastPasswordReset(created, user.getUpdatedAt()));
	}
